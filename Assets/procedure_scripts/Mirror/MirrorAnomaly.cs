...
public class MirrorAnomaly : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("🪞 MirrorAnomaly initialized");
    }
    ...
    {
        Debug.Log($"🪞 Room changed to {roomNumber}");

        
        
        if (isAnomalyActive)
        {
            Debug.Log($"🪞 Mirror already active, updating state for room {roomNumber}");
    ...
    public void ActivateMirrorAnomaly(int roomNumber)
    {
        ...
        if (isAnomalyActive)
        {
            Debug.Log("🪞 Mirror already active, skipping activation");
            return;
        }

        if (mirrorPrefab == null)
        {
            Debug.LogError("❌ Mirror prefab is not assigned!");
            return;
        }
        ...
        {
            Debug.LogError("❌ Camera not found in mirror prefab!");
            return;
        }

        Debug.Log("🪞 Creating mirror from prefab...");
        ...
        if (currentRoom != null)
        {
            ...
            Debug.Log($"📍 Mirror spawned at local position: {mirrorObject.transform.localPosition} relative to room {currentRoom.name}");
        }
        else
        {
            Debug.LogWarning("⚠️ No current room with MirrorAnomaly found, spawning at world origin");
            mirrorObject = Instantiate(mirrorPrefab, Vector3.zero, Quaternion.identity);
            mirrorObject.name = "DynamicMirror_Room" + roomNumber;
            mirrorObject.SetActive(true);
        }

        Debug.Log($"✅ Mirror created: {mirrorObject.name} at position {mirrorObject.transform.position}");
        ...
        {
            Debug.LogError("❌ Mirror camera not found!");
            return;
        }

        if (mirrorRenderer == null)
        {
            Debug.LogError("❌ Mirror renderer not found!");
            return;
        }
        ...
        Debug.Log($"✅ Mirror camera initialized at: {initialCameraPosition}");
        ...
        Debug.Log($"🪞 Mirror activated! Showing: {(isShowingTruth ? "TRUTH" : "LIE")}");
        ...
        if (mirrorActivationSound != null)
        {
            ...
            Debug.Log("🔊 Mirror activation sound played");
        }
        ...
        if (VoiceGuideSystem.Instance != null)
        {
            ...
            Debug.Log("🗣️ Voice guide message queued");
        }
    }
    ...
    void SetupDynamicMirror()
    {
        ...
        if (mirrorRenderer != null)
        {
            ...
            if (originalMirrorMaterial != null)
            {
                ...
                Debug.Log("✅ Render texture assigned to mirror material");
            }
            else
            {
                Debug.LogError("❌ Mirror material is null");
            }
        }
        ...
        if (mirrorMarkersLayer >= 0)
        {
            ...
            Debug.Log($"✅ Hidden MirrorMarkers layer from main camera");
        }

        
        mirrorCamera.Render();

        Debug.Log("✅ Dynamic mirror setup complete");
    }
    ...
    private IEnumerator MirrorAnomalySequence()
    {
        ...
        if (mirrorHumSound != null)
        {
            ...
            audioSource.Play();
            Debug.Log("🔊 Mirror hum sound started");
        }

        Debug.Log("✅ Mirror anomaly sequence complete");
    }
    ...
    private void CreateDoorMarker(Door door, bool isCorrectInMirror)
    {
        try
        {
            ...
            Debug.Log($"✅ Created marker on layer 'MirrorMarkers' for door: {door.name}");
        }
    ...
    private void StopMirrorAnomaly()
    {
        ...
        if (mainCamera != null)
        {
            ...
            {
                mainCamera.cullingMask |= (1 << mirrorMarkersLayer);
                Debug.Log($"✅ Restored MirrorMarkers layer to main camera");
            }
        }

        isAnomalyActive = false;
        Debug.Log("🪞 Mirror anomaly stopped");
    }
