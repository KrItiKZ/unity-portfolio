...
public class Room : MonoBehaviour
{
    ...
    public void SetupRoom(int newRoomNumber)
    {
        ...
        if (isFakeRoom)
        {
            ...
            Debug.Log($"[FakeRoom] Комната {roomNumber} — все двери заблокированы");
            return; 
        }
        ...
        if (currentAnomaly != "UVFlashlightSpawn" && spawnRoomNumber != roomNumber)
        {
            foreach (Transform child in transform)
            {
                if (child.name.Contains("Table")) 
                {
                    ...
                    Debug.Log($"Destroyed UV Table in room {roomNumber} (anomaly: {currentAnomaly}, spawn room: {spawnRoomNumber})");
                }
            }
        }
    }
    ...
    public void ForceSetupRoom(int newRoomNumber)
    {
        ...
        if (AnomalyManager.Instance == null || SessionManager.Instance == null)
        {
            ...
            Debug.LogWarning("Managers not initialized, using SoundHints for room " + roomNumber);
        }
        else
        {
            ...
            if (string.IsNullOrEmpty(currentAnomaly))
            {
                currentAnomaly = "SoundHints";
                AnomalyManager.Instance.currentRoomAnomaly = currentAnomaly;
                Debug.LogWarning("AnomalyManager returned empty anomaly, using SoundHints");
            }
        }

        Debug.Log($"Room {roomNumber} initialized with anomaly: {currentAnomaly}");
    ...
    private IEnumerator SpawnAnomaliesDelayed(string anomalyToSpawn)
    {
        ...
        Debug.Log($"SpawnAnomaliesDelayed called for room {roomNumber} with anomaly: {anomalyToSpawn}");
    }
    ...
    private void SetupCorrectDoors()
    {
        ...
        if (availableDoors.Count > 0)
        {
            ...
            if (clock != null && noteManager != null && noteManager.HasClockInCurrentRoom)
            {
                ...
                Debug.Log($"[ClockPuzzle] Room {roomNumber} | Time 6-9: {shouldForceLeftDoor} | Clock time: {clock.currentHour}:{clock.currentMinute:D2}");
            }
            ...
            if (shouldForceLeftDoor)
            {
                ...
                correctDoorIndex = 0;
                Debug.Log($"[ClockPuzzle] FORCING LEFT DOOR (index 0 after sort)");
            }
            else
            {
